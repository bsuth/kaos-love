...
function lurker.onerror(e, nostacktrace)
  ...
  local stacktrace = nostacktrace and ''
    or lume.trim((debug.traceback('', 2):gsub('\t', '')))
  ...
  love.draw = function()
    ...
    drawtext(
      'lurker',
      width - love.graphics.getFont():getWidth('lurker') - pad,
      pad,
      colors[4]
    )
    drawhr(pad + 32, colors[4], colors[5])
    drawtext(
      'If you fix the problem and update the file the program will ' .. 'resume',
      pad,
      pad + 46,
      colors[3]
    )
